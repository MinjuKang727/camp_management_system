...
//            System.out.println(e.getMessage());
    ...
    private Student getStudentById(String studentId) throws BadInputException{
        ...
        throw new BadInputException("존재하지 않는 수강생 고유 번호입니다.", "수강생 목록 조회의 수강생 고유 번호를 참고하여 입력해 주십시오.");
    }
    ...
    // 수강 중이지 않은 과목만 출력
    private void printSubjectList(String subjectType, List<Subject> subjectList, List<Subject> joinedSubject) {
        System.out.printf("\n[ %s 과목 목록 ]\n", subjectType);
         for (Subject subject : subjectList) {
    ...
    private void joinSubject(String subjectType, Student student, List<Subject> subjectList) {
        ...
        while (flag) {
            ...
            Subject subject = this.inSubjectId(subjectType, subjectList, joinedSubject);
    ...
    private void displayScoreView() {
        ...
        while (flag) {
            ...
            int input = this.enterType("관리 항목을 선택하세요...", 1, 5, 0);
            ...
            switch (input) {
                ...
                case 4 -> this.inquireSubjectAvg();
                case 5 -> this.inquireAverageGradeByStatus();
    ...
    private int enterType(String msg, int min, int max, int exception) {
        while (true) {
            ...
            try {
                ...
                System.out.println(e.getMessage());
                exit = this.enterType("이전 페이지로 돌아가시겠습니까? (돌아가기 : exit 입력)");
            } catch (Exception e) {
                System.out.println("알 수 없는 항목을 입력하셨습니다.");
                exit = this.enterType("이전 페이지로 돌아가시겠습니까? (돌아가기 : exit 입력)");
            }
    ...
    private String enterType(String type) {
        return switch (type) {
            case INDEX_TYPE_STUDENT -> {
                System.out.println("수강생 고유 번호를 입력하십시오... (ST를 제외한 뒤의 번호만 입력)");
                yield String.format("%s%s", INDEX_TYPE_STUDENT, sc.nextLine());
            }
            case INDEX_TYPE_SUBJECT -> {
                System.out.println("과목 고유 번호 를 입력하십시오...(SU를 제외한 뒤의 번호만 입력)");
                yield String.format("%s%s", INDEX_TYPE_SUBJECT, sc.nextLine());
            }
            case INDEX_TYPE_SCORE -> {
                System.out.println("점수 고유 번호 를 입력하십시오...");
    ...
    // 과목 아이디 입력 >> 과목 객체 반환
    // 수강 중이지 않은 과목 선택
    private Subject inSubjectId(String subjectType, List<Subject> subjectList, List<Subject> joinedSubjectList) {
        while (true) {
            this.printSubjectList(subjectType, subjectList, joinedSubjectList);
    ...
    private Subject inSubjectId(Student student, String subjectType) {
        ...
        while (true) {
            ...
            this.printSubjectList(subjectType, student, subjectList);
            String subjectId = this.enterType(INDEX_TYPE_SUBJECT);
            ...
            try {
                ...
                System.out.println("알 수 없는 과목 고유 번호 를 입력하셨습니다.\n hint : 과목 고유 번호 ('SU0')에서 'SU'를 제외한 뒤의 번호만 입력해 주세요.");
            }
    ...
    private String ranked(double score, String subjectType){
    ...
    // 수강생의 과목별 평균 등급 조회
    private void inquireSubjectAvg() {
        System.out.println("\n==================================");
        ...
        this.inquireSubjectAvg(student, SUBJECT_TYPE_MANDATORY);
        this.inquireSubjectAvg(student, SUBJECT_TYPE_CHOICE);
    }

    // 수강생의 과목 타입별 필수 과목 평균 등급 조회
    private void inquireSubjectAvg(Student student, String subjectType) {
        ...
        for (Subject subject : subjectList) {
            ...
            try{
                ...
                String subjectRank = this.ranked(subjectAvg, subjectType);
                System.out.printf("- %s : %s 등급\n", subject.getSubjectName(), subjectRank);
            } catch (BadInputException e) {
                continue;
            }
        }
        ...
        for (Student student : studentList) {
            ...
            if (count == 0) {
                ...
                double averageScore = (double) totalScore / count;
                 // 평균 점수를 기반으로 등급 계산
                String avgRank = this.ranked(averageScore, SUBJECT_TYPE_MANDATORY);
